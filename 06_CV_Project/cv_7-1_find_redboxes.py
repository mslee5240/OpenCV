# OpenCV를 사용하여 이미지에서 빨간색 네모 상자를 감지하고, 감지된 상자의 중심 좌표와 회전 각도를 계산하는 프로그램
# - 입력 이미지를 HSV 색 공간으로 변환하여 빨간색 범위를 마스킹합니다.
# - 윤곽선을 추출하고 최소 바운딩 박스를 계산하여 중심 좌표와 각도를 도출합니다.
...
def find_red_boxes(image):
    """
    이미지에서 빨간색 네모 상자를 감지하고 중심 좌표와 각도를 계산.
    :param image: 입력 이미지 (BGR 형식)
    :return: 빨간 네모 상자의 중심 좌표와 각도 리스트
    """
    # 1-1. 이미지를 HSV 색 공간으로 변환
    hsv = cv2.cvtColor(image, cv2.COLOR_BGR2HSV)

    # 1-2. 빨간색 범위 지정 (OpenCV의 H 값은 0-180 범위) : 빨간색 (0~10 및 170~180) , 파란색 (110~130)
    lower_red = np.array([0, 100, 100])    # 빨간색 하한
    upper_red = np.array([10, 255, 255])   # 빨간색 상한
    lower_red2 = np.array([170, 100, 100]) # 빨간색 하한 (반대쪽 원통 끝)
    upper_red2 = np.array([180, 255, 255]) # 빨간색 상한

    # 1-3. 마스크 생성 (빨간색 영역 선택)
    ...
    for contour in contours:
        # 윤곽선의 최소 바운딩 박스 계산
        rect = cv2.minAreaRect(contour)  # ((중심 좌표), (가로, 세로 크기), 각도)
        print(f'rect : {rect}')

        # 윤곽선 면적 필터링 (노이즈 제거)
        if cv2.contourArea(contour) > 100:  # 면적 기준
            center_x, center_y = map(int, rect[0])  # 중심 좌표
            angle = rect[2]                        # 회전 각도
            red_boxes.append((center_x, center_y, angle))  # 결과 저장
...
for box in red_boxes:
    print("빨간 네모 상자의 중심 좌표:", (box[0], box[1]))
